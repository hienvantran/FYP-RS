...
class BPRLoss:
    ...
    def stageOne(self, sub_graph1, sub_graph2, users, pos, neg):
        # TODO: add subgraph here
        sup_logits, ssl_logits_user, ssl_logits_item = self.model(
                    sub_graph1, sub_graph2, users, pos, neg)
                
        # BPR Loss
        bpr_loss = -torch.sum(F.logsigmoid(sup_logits))

        # Reg Loss
        (_, _, _, userEmb0,  posEmb0, negEmb0) = self.model.getEmbedding(users.long(), pos.long(), neg.long())
